...
export const createPayment = (document, payMoney) => {
  ...
  return {
    ...
    Amount: Math.round(document.doc_gross_amount * 10) / 10, // [required] [Amount is the number after rounding]
    ...
    Change: payMoney - Math.round(document.doc_gross_amount * 10) / 10, // [required] [ActualAmount - Amount]
    RoundingAmount: (Math.round(document.doc_gross_amount * 10) / 10 - document.doc_gross_amount).toFixed(2) * 1, // [required] [Amount of Rounding]
